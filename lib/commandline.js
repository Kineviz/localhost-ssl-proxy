...
program
  ...
  .option('--targetHostname <type>', 'Self-sign SSL DNS/IP (default localhost)',"localhost")
  .option('--proxySSL <type>', 'Proxy SSL Site',false)
  .option('--useCORS <type>', 'Use CORS',true)
  .option('--onlyWS <type>', 'onlyWS',false)
  .option('--staticPath <type>', 'Static web path, default staticPath','staticPath')
  .option('--staticWS <type>', 'Static Live reload websocket path (default /sockjs-node)',"/sockjs-node")
  .option('--proxyServer <type>', 'proxy server (e.g. https://graphxrdev.kineviz.com)')
